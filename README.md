<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=005C84&height=250&section=header&text=Navin%20Rajaa&fontSize=80&fontColor=ffffff&animation=fadeIn&desc=Backend%20Engineer%20|%20Application%20Security%20Enthusiast&descAlignY=75&descAlign=50" width="100%" />

<a href="https://github.com/navinrajaa2">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=2EC866&center=true&vCenter=true&width=600&lines=Building+Scalable+Backend+Systems;Securing+Applications;Mastering+Offensive+Security" />
    <source media="(prefers-color-scheme: light)" srcset="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=005C84&center=true&vCenter=true&width=600&lines=Building+Scalable+Backend+Systems;Securing+Applications;Mastering+Offensive+Security" />
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=2EC866&center=true&vCenter=true&width=600&lines=Building+Scalable+Backend+Systems;Securing+Applications;Mastering+Offensive+Security" alt="Typing SVG" />
  </picture>
</a>

<p align="center">
  <a href="https://www.linkedin.com/in/navin-rajaa-61262a329/"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"></a>
  <a href="https://www.leetcode.com/navinrajaa"><img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=LeetCode&logoColor=black" alt="LeetCode"></a>
  <a href="https://www.hackerrank.com/navinrajaa02"><img src="https://img.shields.io/badge/HackerRank-2EC866?style=for-the-badge&logo=HackerRank&logoColor=white" alt="HackerRank"></a>
  <a href="https://instagram.com/navin_rajaa_2"><img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" alt="Instagram"></a>
  <a href="mailto:navinrajaa02@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"></a>
</p>

</div>

---

<div align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=navinrajaa2&theme=tokyonight&margin-w=15&no-bg=true&no-frame=true&column=7" />
</div>

---

## Executive Summary

I am a Computer Science student dedicated to backend engineering and offensive security. My academic journey and self-directed projects focus on constructing robust, highly scalable systems securely. My overarching goal is to narrow the gap between conventional software engineering and advanced ethical hacking methodologies. I thrive in environments that challenge algorithmic thinking and demand strict adherence to secure coding principles.

```java
public class NavinRajaa {
    private final String role = "Java Backend Developer";
    private final String[] expertise = {"Spring Boot REST APIs", "Microservices", "Application Security"};
    private final String currentFocus = "Red Teaming & Offensive Security";
    
    public void executeDailyRoutine() {
        while (awake) {
            solveAlgorithmicChallenges();
            developScalableSystems();
            analyzeSystemVulnerabilities();
            deployOptimizedSolutions();
        }
    }
}
```

---

## Technical Stack

<div align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=java,spring,js,python,c,html,css,tailwind,react,mysql,mongo,git,linux,vscode,bash,docker,postman&perline=9" />
  </a>
</div>

---

## Core Competencies

<table align="center" width="100%">
  <tr>
    <td width="50%" align="center">
      <h3>Backend Architecture</h3>
      <div align="left">
        <ul>
          <li><strong>Frameworks:</strong> Spring Boot, Object-Oriented Design</li>
          <li><strong>Architecture:</strong> RESTful API Design, Microservices</li>
          <li><strong>Databases:</strong> MySQL, MongoDB, Database Optimization</li>
        </ul>
      </div>
    </td>
    <td width="50%" align="center">
      <h3>Security & Red Teaming</h3>
      <div align="left">
        <ul>
          <li><strong>Web Security:</strong> OWASP Top 10, Penetration Testing</li>
          <li><strong>Network Security:</strong> Protocols, Attack Vectors</li>
          <li><strong>Secure Practices:</strong> Input Validation, Threat Modeling</li>
        </ul>
      </div>
    </td>
  </tr>
  <tr>
    <td width="50%" align="center">
      <h3>Algorithm Design</h3>
      <div align="left">
        <ul>
          <li><strong>Data Structures:</strong> Trees, Graphs, Hash Maps</li>
          <li><strong>Algorithms:</strong> Dynamic Programming, Optimization</li>
          <li><strong>Metrics:</strong> Time/Space Complexity Tuning</li>
        </ul>
      </div>
    </td>
    <td width="50%" align="center">
      <h3>Frontend Engineering</h3>
      <div align="left">
        <ul>
          <li><strong>Libraries:</strong> React.js, Tailwind CSS</li>
          <li><strong>Languages:</strong> JavaScript (ES6+), HTML5, CSS3</li>
          <li><strong>Focus:</strong> Responsive UI, State Management</li>
        </ul>
      </div>
    </td>
  </tr>
</table>

---

## Featured Development

<p align="center">
  <a href="https://github.com/navinrajaa2">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=navinrajaa2&repo=navinrajaa2&theme=tokyonight&bg_color=0D1117&hide_border=true&title_color=2EC866" width="48%" />
  </a>
  <a href="https://github.com/navinrajaa2">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=navinrajaa2&repo=portfolio&theme=tokyonight&bg_color=0D1117&hide_border=true&title_color=2EC866" width="48%" />
  </a>
</p>

*Note: Change `repo=portfolio` to your actual top repositories.*

---

## Quantitative Performance

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=navinrajaa2&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=2EC866" height="192" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=navinrajaa2&layout=donut&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=2EC866&card_width=350" height="192" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=navinrajaa2&theme=tokyonight&hide_border=true&background=0D1117&ring=2EC866&fire=2EC866&currStreakLabel=2EC866" height="192" />
</p>

<p align="center">
  <img src="https://leetcard.jacoblin.cool/navinrajaa?theme=dark&ext=heatmap" width="80%" />
</p>

---

## Activity & Progression

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=navinrajaa2&theme=tokyo-night&bg_color=0D1117&hide_border=true&height=320&line=2EC866&point=FFFFFF" width="100%" />
</p>

<br>

<p align="center">
  <img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight&bg_color=0D1117&border=false" />
</p>

---

## Engineering Philosophy

> "Secure systems are not assembled by accident; they are the result of rigorous architectural planning, continuous testing, and an adversarial mindset integrated into the development lifecycle."

### Strategic Objectives for 2026

- Architect and deploy production-grade Spring Boot microservices.
- Cross the milestone of solving 300+ advanced LeetCode challenges.
- Publish security research on common backend vulnerabilities.
- Acquire certifications in ethical hacking and penetration testing.
- Contribute sophisticated modules to open-source security tools.

---

<div align="center">
  <a href="https://github.com/navinrajaa2">
    <img src="https://komarev.com/ghpvc/?username=navinrajaa2&color=005C84&style=flat-square&label=PROFILE+VIEWS" alt="Profile Views" />
  </a>
</div>
